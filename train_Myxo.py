...
def subprocess_fn(rank, c, temp_dir):
    ...
    if c.num_gpus > 1:
        ...
        if os.name == 'nt':
            init_method = "file:///{0}".format(init_file.replace('\\', '/'))
    ...
    new_training_loop.training_loop(rank=rank, **c)
...
@click.option('--freezed', help='Freeze first layers of D', metavar='INT', type=click.IntRange(min=0), default=0,
              show_default=True)
@click.option('--citers', help='Train the D for citers in each epoch', metavar='INT', default=5,
...
@click.option('--dlr', help='D learning rate', metavar='FLOAT', type=click.FloatRange(min=0), default=0.001,
              show_default=True)
...
@click.option('--snap', help='How often to save snapshots', metavar='TICKS', type=click.IntRange(min=1), default=1,
              show_default=True)
@click.option('--seed', help='Random seed', metavar='INT', type=click.IntRange(min=0), default=108, show_default=True)
...
def main(**kwargs):
    ...
    c.G_kwargs = dnnlib.EasyDict(class_name=None, z_dim=opts.z_dim, w_dim=512, mapping_kwargs=dnnlib.EasyDict())
    c.D_kwargs = dnnlib.EasyDict(class_name='training.networks_stylegan2.Discriminator2', z_dim=opts.z_dim,
    ...
    if opts.validation_data:
        ...
        c.validation_set_kwargs.random_seed = opts.seed
        c.validation_data_loader_kwargs.num_workers = opts.workers

    # Hyperparameters & settings.
    c.citers = opts.citers
    ...
    # Construct Encoder
    c.E_kwargs = copy.deepcopy(c.D_kwargs)
    c.D_kwargs.model_name = 'discriminator'
    c.E_kwargs.model_name = 'encoder'
