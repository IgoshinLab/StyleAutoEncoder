...
class InfiniteSampler(torch.utils.data.Sampler):
    def __init__(self, dataset, rank=0, num_replicas=1, shuffle=True, seed=0, window_size=0.5):
        ...
        if self.dataset.label_dict:
            ...
            pheno_clss = list(sample_dict.keys())
            pheno_clss.sort()
            fname_idx = {}
            for idx, fname in enumerate(self.dataset._image_fnames):
                fname_idx[fname] = idx  # Construct the dict in O(n)
            for pheno_cls in pheno_clss:  # Construct the stratified sampling dict in O(n)
                self.stratified_dict[pheno_cls] = [fname_idx[sample_name] for sample_name in sample_dict[pheno_cls]]
            del fname_idx
